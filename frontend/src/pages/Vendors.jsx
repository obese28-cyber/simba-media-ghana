...
import { fmt, BRAND, ACCENT } from '../utils'
...
export default function Vendors() {
  ...
  const load = () => {
    apiFetch('/api/vendors').then(r => r.json()).then(setRows)
    apiFetch('/api/vendor-balances').then(r => r.json()).then(setBalances)
  }
  useEffect(() => { load() }, [])
  ...
  const del  = async (id) => { if (!confirm('Delete?')) return; await apiFetch(`/api/vendors/${id}`, { method: 'DELETE' }); load() }
  ...
  const grandTotal = balances.reduce((s, b) => s + b.total, 0)
  const maxBal     = balances[0]?.total || 1
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20, alignItems: 'start' }}>
      ...
      <div>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', marginBottom: 24, border: `2px solid ${BRAND}` }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 1.2fr 1.5fr 1.5fr', gap: 12 }}>
              <div><label style={lbl}>Company / Name *</label>
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Vendor name" style={inp} /></div>
              <div><label style={lbl}>Contact Person</label>
                <input value={form.contact} onChange={e => setForm(f => ({ ...f, contact: e.target.value }))} placeholder="Contact name" style={inp} /></div>
              <div><label style={lbl}>Phone</label>
                <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+233…" style={inp} /></div>
              <div><label style={lbl}>Email</label>
                <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="email@…" style={inp} /></div>
              <div><label style={lbl}>Service Type</label>
                <input value={form.service_type} onChange={e => setForm(f => ({ ...f, service_type: e.target.value }))} placeholder="e.g. Equipment Hire" style={inp} /></div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 14 }}>
          ...
          {rows.map(v => (
            <div key={v.id} style={{ background: '#fff', borderRadius: 12, padding: '16px 18px', boxShadow: '0 1px 8px rgba(0,0,0,0.07)', border: '1px solid #f1f5f9' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 14, color: BRAND }}>{v.name}</div>
              ...
              {v.contact && <div style={{ fontSize: 12, color: '#475569', marginTop: 4 }}>👤 {v.contact}</div>}
              {v.phone   && <div style={{ fontSize: 12, color: '#475569', marginTop: 3 }}>📞 {v.phone}</div>}
              {v.email   && <div style={{ fontSize: 12, color: '#475569', marginTop: 3 }}>✉️ {v.email}</div>}
              {v.notes   && <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 8, borderTop: '1px solid #f1f5f9', paddingTop: 8 }}>{v.notes}</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ position: 'sticky', top: 20 }}>
        <div style={{ background: BRAND, borderRadius: 14, padding: '20px', marginBottom: 14, boxShadow: '0 4px 20px rgba(10,61,98,0.2)' }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: ACCENT, textTransform: 'uppercase', letterSpacing: '1.5px', margin: '0 0 10px' }}>All Time</p>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', margin: '0 0 4px' }}>Total Paid to Vendors</p>
          <p style={{ fontSize: 26, fontWeight: 900, color: '#fff', margin: 0 }}>{fmt(grandTotal)}</p>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 6 }}>{rows.length} vendors</p>
        </div>

        <div style={{ background: '#fff', borderRadius: 14, padding: '18px 20px', boxShadow: '0 1px 8px rgba(0,0,0,0.07)' }}>
          <p style={{ fontSize: 11, fontWeight: 800, color: BRAND, textTransform: 'uppercase', letterSpacing: '0.8px', margin: '0 0 16px' }}>Vendor Spend</p>
          {balances.filter(b => b.total > 0).length === 0 && <p style={{ color: '#94a3b8', fontSize: 13, margin: 0 }}>No spend recorded</p>}
          {balances.filter(b => b.total > 0).map(b => (
            <div key={b.id} style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontSize: 12, color: '#374151', fontWeight: 700 }}>{b.name}</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: BRAND }}>{fmt(b.total)}</span>
              </div>
              <div style={{ height: 6, background: '#f1f5f9', borderRadius: 3 }}>
                <div style={{ height: '100%', width: `${(b.total/maxBal)*100}%`, background: ACCENT, borderRadius: 3 }} />
              </div>
              <div style={{ display: 'flex', gap: 12, marginTop: 3 }}>
                {b.direct_costs > 0 && <span style={{ fontSize: 10, color: '#64748b' }}>Direct: {fmt(b.direct_costs)}</span>}
                {b.expenses > 0    && <span style={{ fontSize: 10, color: '#64748b' }}>Admin: {fmt(b.expenses)}</span>}
              </div>
            </div>
          ))}
        </div>

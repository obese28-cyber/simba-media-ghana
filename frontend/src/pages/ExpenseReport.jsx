...
import { fmt, MONTHS, YEARS, CURRENT_YEAR, BRAND, ACCENT, EXPENSE_CATS } from '../utils';
...
export default function ExpenseReport() {
  ...
  useEffect(() => {
    apiFetch('/api/expenses?year=' + year).then(r => r.json()).then(setRows);
  }, [year]);
  ...
  var activeCats = EXPENSE_CATS.filter(function(cat) { return catTotal(cat) > 0; });
  ...
  function printReport() {
    ...
    w.document.write('<!DOCTYPE html><html><head>' +
      '<title>Expense Report ' + year + ' — Simba Media Ghana</title>' +
      '<style>' +
      ...
      '<table><thead><tr>' +
      '<th class="lh" style="min-width:140px">Expenses</th>' +
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", color: '#1e293b' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: BRAND, margin: 0 }}>Expense Analysis Report</h1>
